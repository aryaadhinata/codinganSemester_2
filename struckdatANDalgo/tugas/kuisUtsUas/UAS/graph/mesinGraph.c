...
void addSimpul(char temp[], graph *G){
    ...
    strcpy(baru->kontainer.konten, temp);
    baru->kontainer.mark = 0;
...
simpul* findSimpul(char konten[], graph G){
    ...
    while((bantu != NULL) && (ketemu == 0)){
        ...
        if(strcmp(bantu->kontainer.konten, konten) == 0){
            hasil = bantu;
            ketemu = 1;
        }else{
...
void delJalur(char tuju[], simpul *awal){
    ...
    if(hapus != NULL){ // ada yang bisa dihapus
        ...
        while((hapus != NULL) && (ketemu == 0)){ // kalo belum ditelusuri semua dn belum ketemu
            ...
            if(strcmp(hapus->tujuan->kontainer.konten, tuju) == 0){
                ketemu = 1;
            }else{
                prev = hapus;
...
void delSimpul(char temp[], graph *G){
    ...
    if(hapus != NULL){
        ...
        while((hapus != NULL) && (ketemu == 0)){
            ...
            if(strcmp(hapus->kontainer.konten, temp) == 0){
                ketemu = 1;
            }else{
                prev = hapus;
        ...
        if(ketemu == 1){
            ...
            while(bantu != NULL){
                ...
                if(bantu != hapus){
                    delJalur(hapus->kontainer.konten, bantu);
                }
                bantu = bantu->nextSimpul;
            }
        }
...
void resetVisit(graph *G){
    simpul *bantu = G->first;
    while(bantu != NULL){
        if(bantu->kontainer.mark == 1){
            bantu->kontainer.mark = 0;
        }
        bantu = bantu->nextSimpul;
    }
}
...
void jalan(char begin[], char end[], simpul *node){
    ...
    if((bantu != NULL) && ){
        while(strcmp(bantu->kontainer.konten, begin) == 0){
            jalur *help = bantu->arc;
            bantu->kontainer.mark = 1;
            jalan(begin, end, help->tujuan);
            help = help->nextJalur;
        }
        bantu = bantu->nextSimpul;
    }
